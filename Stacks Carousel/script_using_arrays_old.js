...
/*
    We want to iterate through a list of objects
    each object contains data in the format
    obj {
        Image-URL: '';
        Title: '';
        Content: '';
...
function InitializeQueue() {
    //Move first four elements onto queue.
    for(i=0; i<4; i++) {
...
function onClick( dir ) {
    ...
    if( dir == '<' ) {
        for( i=3; i>=0; i-- ) {
            if( queue[i].id == 0 ) {
                queue[i] = elements[elements.length-1];
            } else {
                queue[i] = elements[queue[i].id-1];
            }
        }
    } else {
        for( i=0; i<4; i++ ) {
            if( queue[i].id == elements.length-1 ) {
                queue[i] = elements[0];
            } else {
                queue[i] = elements[queue[i].id+1];
            }
